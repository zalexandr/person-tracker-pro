class PersonTrackerProCard extends HTMLElement {
  ...
  getGridOptions() { return { rows: 6, columns: 12, min_rows: 4, min_columns: 6, max_columns: 12 }; }

  static getStubConfig() {
    return { location_entity: "", show_map: true, hours_to_show: 24, map_zoom: 14, show_history: true };
  }
  ...
  static getConfigForm() {
    return {
      ...
      computeLabel: (schema) => ({
        location_entity: "Location entity", confidence_entity: "Presence confidence", accuracy_entity: "GPS accuracy",
        speed_entity: "Speed", sources_entity: "Active sources", rejected_entity: "Rejected GPS samples",
        moving_entity: "Moving", stale_entity: "Location stale", offline_entity: "Location offline",
        show_map: "Show map", hours_to_show: "Map history (hours)", map_zoom: "Map zoom", show_history: "Show history",
      })[schema.name],
    };
  }
  ...
  async _renderMap(host, config) {
    ...
    const map = document.createElement("hui-map-card");
  ...
  _render() {
    ...
    const rows = [
      ["Confidence", this._value(c.confidence_entity)], ["GPS accuracy", this._value(c.accuracy_entity)],
      ["Speed", this._value(c.speed_entity)], ["Active sources", this._value(c.sources_entity)],
      ["Rejected samples", this._value(c.rejected_entity)], ["Moving", this._binary(c.moving_entity)],
      ["Stale", this._binary(c.stale_entity)], ["Offline", this._binary(c.offline_entity)],
    ];

    this.innerHTML = `
      <ha-card>
        <div class="header"><ha-icon icon="mdi:map-marker-account"></ha-icon>
        ...
        <div class="grid">${rows.filter(([, value]) => value !== "—").map(([label, value]) => `<div class="metric"><div class="label">${label}</div><div class="value">${this._escape(value)}</div></div>`).join("")}</div>
        <div class="footer"><span>${this._escape(attrs.zone || location?.state || "Unknown zone")}</span>${attrs.last_update ? `<span>Updated ${this._escape(attrs.last_update)}</span>` : ""}</div>
      </ha-card>`;
    this._style();
    if (showMap) this._renderMap(this.querySelector("#tracker-map"), {
      type: "map", entities: [c.location_entity], hours_to_show: Number(c.hours_to_show || 24),
      default_zoom: Number(c.map_zoom || 14), auto_fit: true,
    });
  }
  ...
  _style() {
    ...
    style.textContent = `
      ...
      .header ha-icon { color:var(--primary-color); } .title { font-weight:600; flex:1; }
      .state { color:var(--secondary-text-color); text-transform:capitalize; }
      .map { height:260px; } .map > hui-map-card { display:block; height:100%; }
      .grid { display:grid; grid-template-columns:repeat(4,minmax(0,1fr)); gap:1px; background:var(--divider-color); }
      .metric { padding:12px; background:var(--card-background-color); } .label { font-size:12px; color:var(--secondary-text-color); }
...
window.customCards.push({
  ...
  description: "Location, confidence, GPS, movement and status in one card.", preview: true,
  ...
  getEntitySuggestion: (hass, entityId) => {
    ...
    return { config: { type: "custom:person-tracker-pro-card", location_entity: entityId, show_map: true, hours_to_show: 24, map_zoom: 14, show_history: true } };
  },
});
